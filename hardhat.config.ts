...
const config: HardhatUserConfig = {
  ...
  networks: {
    megaethTestnet: {
      ...
      accounts: process.env.MEGAETH_PRIVATE_KEY
        ? [process.env.MEGAETH_PRIVATE_KEY]
        : [],
      gasPrice: process.env.MEGAETH_GAS_PRICE
        ? parseInt(process.env.MEGAETH_GAS_PRICE, 10)
        : undefined
    }
  },
  gasReporter: {
