...
async function findPairCreationBlock(pairAddress) {
  const codeABI = [{ inputs: [], name: 'token0', outputs: [{ name: '', type: 'address' }], stateMutability: 'view', type: 'function' }];
  ...
  for (const range of ranges) {
    try {
      ...
      console.log(`  ✗ Pair did NOT exist at block ${range.end} (${range.name})`);
    }
  }
...
async function main() {
  ...
  for (let i = 0; i < pairCount; i++) {
    ...
    const token0 = getContract({ address: token0Address, abi: erc20ABI, client });
    const token1 = getContract({ address: token1Address, abi: erc20ABI, client });
    ...
    if (range) {
      if (range.start < 21684900) {
        console.log(`  ⚠️  CREATED BEFORE STARTBLOCK - Will NOT be indexed by subgraph!`);
      } else {
